#!/bin/bash
# Bliss d12 SFT using the exact runtime prompt scaffold shipped in nc_run.c:
#   You are Bliss, ...
#   Q: <prompt>
#   A:<answer>\n
#
# BLISS_NUM_ITERATIONS is nanochat data-generator iterations, not optimizer
# steps. With default grad_accum_steps=8, 800 ~= 100 optimizer updates.
set -euo pipefail

BLISS_REPO_DIR="${BLISS_REPO_DIR:-$HOME/bliss-sft-v1}"
NANOCHAT_DIR="${NANOCHAT_DIR:-$HOME/nanochat}"
BLISS_MODEL_TAG="${BLISS_MODEL_TAG:-d12_runtime_v1}"
export NANOCHAT_BASE_DIR="${NANOCHAT_BASE_DIR:-$HOME/.cache/nanochat}"
export NANOCHAT_DTYPE=float32
export BLISS_PLAIN_CHAT=1
export BLISS_PLAIN_STYLE=qa_nospace
export BLISS_SYSTEM_PREFIX="${BLISS_SYSTEM_PREFIX:-You are Bliss, a small local chat assistant on Windows XP. Answer in one short factual sentence.}"
...
BLISS_TOTAL_BATCH_SIZE="${BLISS_TOTAL_BATCH_SIZE:-8192}"
BLISS_EMBEDDING_LR="${BLISS_EMBEDDING_LR:-0.000005}"
BLISS_UNEMBEDDING_LR="${BLISS_UNEMBEDDING_LR:-0.000010}"
BLISS_MATRIX_LR="${BLISS_MATRIX_LR:-0.000001}"
...
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --plain-style "$BLISS_PLAIN_STYLE" --system-prefix "$BLISS_SYSTEM_PREFIX" --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_SFT_JSONL" --max-tokens 256
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --plain-style "$BLISS_PLAIN_STYLE" --system-prefix "$BLISS_SYSTEM_PREFIX" --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_VAL_JSONL" --max-tokens 256
