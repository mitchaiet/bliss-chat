#!/bin/bash
# Conservative Bliss d12 Q/A SFT. This matches the live runtime prompt style:
#   Q: <prompt>
#   A:<answer>\n
#
# Difference from sft-d12-qa-v1.sh:
# - no space after A:
# - no first-token / newline over-weighting
# - 10x-25x lower full-model learning rates
# - short run by default so behavior can be probed before any export
set -euo pipefail

BLISS_REPO_DIR="${BLISS_REPO_DIR:-$HOME/bliss-sft-v1}"
NANOCHAT_DIR="${NANOCHAT_DIR:-$HOME/nanochat}"
BLISS_MODEL_TAG="${BLISS_MODEL_TAG:-d12_qa_gentle_v1}"
...
export BLISS_GRAD_CLIP="${BLISS_GRAD_CLIP:-0.20}"
export BLISS_ABORT_LOSS="${BLISS_ABORT_LOSS:-8.0}"
export BLISS_FIRST_TOKEN_WEIGHT=1.0
export BLISS_END_TOKEN_WEIGHT=1.0
BLISS_TOTAL_BATCH_SIZE="${BLISS_TOTAL_BATCH_SIZE:-8192}"
BLISS_EMBEDDING_LR="${BLISS_EMBEDDING_LR:-0.00001}"
BLISS_UNEMBEDDING_LR="${BLISS_UNEMBEDDING_LR:-0.00002}"
BLISS_MATRIX_LR="${BLISS_MATRIX_LR:-0.000002}"
BLISS_NUM_ITERATIONS="${BLISS_NUM_ITERATIONS:-200}"
...
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --plain-style "$BLISS_PLAIN_STYLE" --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_SFT_JSONL" --max-tokens 256
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --plain-style "$BLISS_PLAIN_STYLE" --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_VAL_JSONL" --max-tokens 256
