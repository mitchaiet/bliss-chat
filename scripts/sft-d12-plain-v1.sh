#!/bin/bash
# Bliss d12 plain-text SFT. Avoids chat-special tokens; renders:
#   User: <prompt>
#   Assistant: <answer>\n
set -euo pipefail
...
BLISS_MODEL_TAG="${BLISS_MODEL_TAG:-d12_plain_v1}"
export NANOCHAT_BASE_DIR="${NANOCHAT_BASE_DIR:-$HOME/.cache/nanochat}"
export NANOCHAT_DTYPE=float32
export BLISS_PLAIN_CHAT=1
export BLISS_PLAIN_STYLE=user_assistant
...
export BLISS_FIRST_TOKEN_WEIGHT="${BLISS_FIRST_TOKEN_WEIGHT:-4.0}"
export BLISS_END_TOKEN_WEIGHT="${BLISS_END_TOKEN_WEIGHT:-8.0}"
...
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_SFT_JSONL" --max-tokens 512
python "$BLISS_REPO_DIR/tools/check_bliss_sft_data.py" --plain --nanochat "$NANOCHAT_DIR" --jsonl "$BLISS_VAL_JSONL" --max-tokens 512
